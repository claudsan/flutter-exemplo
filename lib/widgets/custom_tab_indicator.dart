...
class RoundedRectangleTabIndicator extends Decoration{
  ...
  RoundedRectangleTabIndicator({@required Color? color, required double? weight, @required double? width})
      : _painter = _RRectablePainterColor(color!, weight!, width!);

  @override
  BoxPainter createBoxPainter([VoidCallback? onChanged]) => _painter;
}

class _RRectablePainterColor extends BoxPainter{
  final Paint _paint;
  final double weight;
  final double width;

  _RRectablePainterColor(Color color, this.weight, this.width)
  : _paint = Paint()
  ..color = color
  .. isAntiAlias = true;
